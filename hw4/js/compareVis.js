...
/**
 * CompareVis object for HW4
 * @param _parentElement -- the (D3-selected) HTML or SVG element to which to attach the vis
 * @param _data -- the data array
 * @param _metaData -- the meta-data / data description object
 * @constructor
 */
function CompareVis (_parentElement, _data, _metaData) {
    /**
     * A word about "this":
     *
     * The meaning of "this" can change from function call to function call; the way
     * we have implemented the class, "this" refers to the CompareVis class
     * in each of the CompareVis.prototype.xxxxxxxx = functions.
 ...
 */
CompareVis.prototype.wrangleData = function (_filterFunction) {
    var self = this;
    
    // displayData should hold the data which is visualized
    self.tempData = self.filterAndAggregate(_filterFunction);
 ...
 * @param selection
 */
CompareVis.prototype.onSelectionChange = function (selectionStart, selectionEnd) {
    var self = this;
 ...
 * @returns {Array|*}
 */
CompareVis.prototype.filterAndAggregate = function (_filter) {
    var self = this;
    ...
    function filteredData(data) {
        if(filter(data)){
            data.prios.forEach(function(elem, index) {
                countArray[index] += elem;
            });
         }
    }

    // create an array of values for the priorities 0-15
    var countArray = new Array(16).fill(0);
